...
function subprocessorsPage(req) {
  const subprocessors = [
    ['Stripe', 'Payments, subscriptions, invoices, customer portal', 'Only when Stripe secrets and price IDs are configured', 'Billing metadata, checkout sessions, subscription status'],
    ['OpenAI-compatible AI provider', 'Optional external generation provider', 'Only when OPENAI_API_KEY is configured', 'Prompts needed to generate requested launch outputs'],
    ['Deployment host / database provider', 'Runs the web app and stores production database/backups', 'Chosen by the operator before production launch', 'Application logs, account records, generated kits, support records'],
    ['GitHub', 'Future code/repository connector', 'Prepared, not live until OAuth/App credentials are configured', 'Repository metadata and actions approved by the customer'],
    ['Shopify', 'Future ecommerce/store connector', 'Prepared, not live until Shopify OAuth credentials are configured', 'Store metadata and approved ecommerce context'],
    ['Google APIs', 'Future Drive/Gmail/Calendar connector', 'Prepared, not live until Google OAuth credentials are configured', 'Approved files, email drafts, calendar metadata, depending on scopes'],
  ];
  return renderPage(req, {
    title: 'Subprocessors and Connected Providers',
    active: '/trust-center',
    description: 'UltraLaunch AI subprocessors and connected provider transparency, including which providers are active, optional, or prepared only.',
    content: `<section class="page-hero premium-page"><div class="container"><p class="eyebrow">Subprocessors</p><h1>Provider transparency without fake enterprise claims.</h1><p>This page explains which external providers may process data, what they are used for, and whether they are active by default. Prepared connectors are not live until official credentials and scopes are configured.</p></div></section>
    <section class="section"><div class="container stack"><div class="card table-card"><h2>Provider register</h2>${table(['Provider','Purpose','Status','Potential data'], subprocessors)}</div><div class="admin-grid"><div class="card"><h2>Current local default</h2><p>The app can run with the offline launch engine and local SQLite storage. In that mode, no external LLM is required. Stripe and OAuth providers remain inactive until configured with environment variables.</p></div><div class="card"><h2>Change policy</h2><p>Before adding a new live provider, document its purpose, data categories, credentials, permissions, business value, and security risks. Do not paste secrets into chat, code, or Git.</p><a class="text-link" href="/privacy">Read privacy policy →</a></div></div></div></section>`,
  });
}
...
function trustCenterPage(req, { snapshot, permissions, message = '', error = '' }) {
  return renderPage(req, {
    ...
    <section class="section"><div class="container stack">${flashHtml(message, 'success')}${flashHtml(error, 'error')}
      ...
      <div class="admin-grid"><form class="card form-card premium-form" method="post" action="/trust/privacy-requests">${csrf(req)}<div><p class="eyebrow">Privacy request center</p><h2>Ask for access, correction, deletion, or restriction.</h2><p class="muted-text">Use instant controls for memories/kits above. Use this queue when a human review is required for account-wide, billing, legal, or support-related records.</p></div><label>Request type<select name="requestType"><option value="access_export">Access / export help</option><option value="delete_account_data">Delete account-owned data</option><option value="correct_data">Correct inaccurate data</option><option value="restrict_processing">Restrict processing</option><option value="billing_data_review">Billing data review</option></select></label><label>Details<textarea name="details" required rows="5" maxlength="1600" placeholder="Tell us what data or processing you want reviewed."></textarea></label><button class="btn" type="submit">Submit privacy request</button></form><div class="card table-card"><h2>Your privacy requests</h2>${snapshot.privacyRequests.length ? table(['Type','Status','Response','Updated'], snapshot.privacyRequests.map((item) => [item.request_type, item.status, item.response_note || '—', formatDate(item.updated_at)])) : '<p>No privacy requests yet.</p>'}</div></div>
...
function adminPage(req, { metrics, users, tickets, events, pilotRequests = [], privacyRequests = [], orders = [], message = '' }) {
  return renderPage(req, {
    title: 'Admin',
    active: '/admin',
    footerCta: false,
    ...
    <section class="section"><div class="container stack">
      ...
      <div class="card table-card"><h2>Privacy requests</h2>${privacyRequests.length ? privacyRequests.map((request) => `<form class="ticket-admin" method="post" action="/admin/privacy-requests/${request.id}/status">${csrf(req)}<div><strong>${e(request.request_type)} · ${e(request.name)}</strong><p>${e(request.details)}</p><span>${e(request.email)} · ${e(request.status)} · ${formatDate(request.created_at)}</span></div><select name="status"><option ${request.status === 'received' ? 'selected' : ''}>received</option><option ${request.status === 'reviewing' ? 'selected' : ''}>reviewing</option><option ${request.status === 'waiting_for_user' ? 'selected' : ''}>waiting_for_user</option><option ${request.status === 'completed' ? 'selected' : ''}>completed</option><option ${request.status === 'declined' ? 'selected' : ''}>declined</option></select><input name="responseNote" maxlength="800" placeholder="Response note" value="${e(request.response_note || '')}"><button class="btn btn-small" type="submit">Update</button></form>`).join('') : '<p>No open privacy requests.</p>'}</div>
