...
function renderPage(req, { title, description = 'AI launch workspace for founders and agencies.', active = '/', content, footerCta = true }) {
  ...
  const authenticatedLinks = user
    ? `<a class="nav-link ${currentPath.startsWith('/dashboard') ? 'active' : ''}" href="/dashboard">Dashboard</a>
       ...
       <a class="nav-link ${currentPath.startsWith('/trust') ? 'active' : ''}" href="/trust">Trust</a>
...
function featuresPage(req) {
  return renderPage(req, {
    ...
    <section class="section"><div class="container feature-matrix">
      ...
      ${featureCard('AI agents and memory', 'Role-based agents remember useful launch context, queue tasks, show KPIs, and keep logs tied to user permissions.')}
      ${featureCard('Automation engine', 'Triggers, conditions, AI actions, draft-only rules, and human approval gates prepare the product for background workers.')}
      ${featureCard('Trust center', 'Customers can inspect permissions, action history, connected-account readiness, memory controls, privacy export, and deletion options.')}
  ...
  return renderPage(req, {
    ...
    <section class="section dashboard-section"><div class="container dashboard-grid">
      <aside class="stack">
        ...
        <div class="card"><p class="eyebrow">AI workforce</p><h2>Agents, automations, trust</h2><p>Your launch workspace now includes permissioned agents, action history, AI memory, exports, and approval queues.</p><div class="mini-link-grid"><a href="/agents">AI agents</a><a href="/automations">Automations</a><a href="/trust">Trust center</a></div></div>
...
function agentsPage(req, { workspace, message = '', error = '' }) {
  return renderPage(req, {
    ...
    content: `<section class="dashboard-hero premium-page"><div class="container dashboard-head"><div><p class="eyebrow">AI Workforce</p><h1>Five AI employees. Permissioned by design.</h1><p>Agents have roles, goals, tools, permissions, memory, tasks, logs, and KPIs. Critical computer-control actions remain approval-gated.</p></div><a class="btn btn-secondary" href="/trust">Trust center</a></div></section>
